...
def Vr_SSM(D, Vh, Tr, Ts, ac, C, Zd, Zr):
    T = Tr + Ts
    Ctot = C + Zd + Zr
...
Spmax = SSM_calculation(Vrinitial, Vh, Tr, Ts, ac, C, Zd, Zr)
SpSafeVal, SpPFLVal = 0, 0
Sp = Spmax
Scurrent = Spmax
SpminVal = Spmin(C, Zd, Zr)
...
#            velR = velXYZ(XnRob, XnRob_last, ts)
            ...
            if faces:
                with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
                    ...
                    try:
                        ...
                        SpminVal = Spmin(C, Zd, Zr)
                        SpSafeVal = SpSafe(Vr_PFL, Ts, ac, C, Zd, Zr)
                        SpPFLVal = SpPFL(Vr_PFL, Vh, Tr, Ts, ac, C, Zd, Zr)

                        # ===== SSM calculation ======
                        Sp = SSM_calculation(Vrinitial, Vh, Tr, Ts, ac, C, Zd, Zr)
                        ...
                        if D <= SpminVal:
                            ...
                            t.sleep(0.5)
                        elif SpminVal < D and SpSafeVal >= D:
                            #server.resume()
                            ...
                            # Vr = Vr_max_command
                            speed = Vr_SSM2(D, Tr, Ts, ac, C, Zd, Zr)
                            Vr = round(speed, 2)
                            print("change value speed safe: ", speed)
                            ...
                            t.sleep(0.5)
                        elif SpSafeVal < D and SpPFLVal >= D:
                            # server.resume()
                            # print("Robot speed reduction")
                            mode_collab = 2
                            # calculate the Vmax allowable
                            ...
                            speed = Vr_PFL
                            Vr = round(speed, 2)
                            print("change value speed PFL: ", speed)
                            # jacoRobot.setSpeed(Vr, vrot)
                            ...
                            # print("Vmax allowable in this workspace: ", Vr_max_command)
                            # Vr = Vr_max_command
                            speed = Vr_SSM(D, Vh, Tr, Ts, ac, C, Zd, Zr)
                            Vr = round(speed, 2)
            ...
            text_currval = font_reg.render(str(D) + " mm", True, (50, 50, 50))
            ...
            output.write(str(interval) + ',' + str(D) + ',' + str(VrOriSSM) + ',' + str(Vr) + ',' + str(Sp) + ',' + str(mode_collab) + ',' + str(XnRob[0]) + ',' + str(XnRob[1]) + ',' + str(XnRob[2]) + '\n')
